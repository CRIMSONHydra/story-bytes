 ...
 * output; bounded retry so the worst case stays well under the 120s nginx timeout.
 */
...
export const generateImage = async (prompt: string, attempts = 2): Promise<GeneratedImage> => {
  ...
  for (let i = 0; i < attempts; i++) {
    try {
      const response = await genAI().models.generateContent({ model: IMAGE_GEN_MODEL, contents: prompt });
      const image = extractInlineImage(response as never);
