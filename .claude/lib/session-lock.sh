...
#
# Multiple Claude windows can be open against the same repo. The validation
# markers under .claude/.validated/ are filesystem-shared, but the heavy
# gates (/audit-feature, /qa, coderabbit-fixer fanout) have no concept of
# "another window is already running this." Without coordination, every
# window's Stop hook hard-blocks and re-runs the same gate -> duplicated
# token spend + duplicated Playwright drives.
#
...
# window can never deadlock the gate.
#
# Lock representation: a directory .claude/.validated/<gate>.lock/ (atomic
...
#     Exit 0 on success (fresh lock taken). Exit 0 with a "stolen stale
#     lock" notice if the existing lock is older than TTL. Exit 2 if
#     another live session holds it; prints the held-by line on stderr.
#
...
#     always.
#
# Default TTL per gate (overridable via --ttl):
#   audit-feature       900   (15 min)
#   qa                  1800  (30 min)
#   frontend / backend  180   (3 min)
#   cr-fixer:*          600   (10 min)
...
lock_root="$repo_root/.claude/.validated"
mkdir -p "$lock_root" 2>/dev/null || true

default_ttl_for() {
  case "$1" in
    audit-feature)        echo 900   ;;
    qa)                   echo 1800  ;;
    frontend|backend)     echo 180   ;;
    cr-fixer:*)           echo 600   ;;
    *)                    echo 600   ;;
  esac
}
...
cmd_acquire() {
  ...
  # Same-session re-acquire is idempotent.
  if [ "$owner_sid" = "$sid" ]; then
    # Refresh the timestamp so the lock doesn't go stale under us.
    printf '%s|%s|%s|%s\n' "$sid" "$(now_unix)" "$$" "$note" > "$lock_d/owner"
    return 0
  fi
...
cmd_release() {
  ...
  # Lock is held by someone else — leave it alone.
